...
namespace Celeste.Mod.Grabhelper {
    ...
    public class SaveDisplayInJournal : OuiJournalPage {
        ...
        public SaveDisplayInJournal(OuiJournal journal, string titleDialogId) : base(journal) {
            PageTexture = "page";
        ...
        protected int GetBestCountFor(AreaKey areaKey) {
            if (ModSaveData.GrabCountPerLevel.TryGetValue(areaKey.GetSID(), out Dictionary<AreaMode, int> grabCounts)) {
                if (grabCounts.TryGetValue(areaKey.Mode, out int grabCount)) {
                    return grabCount;
                }
            }
            return 0;
